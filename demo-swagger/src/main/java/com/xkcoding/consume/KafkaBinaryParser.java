...
public class KafkaBinaryParser {
                ...
                for (long i = 0; i < teimRecord && pos < s_dataLen; i++) {
                    for (TestField f : fieldT) {
                        ...
                        switch (f.type) {
                            ...
                            case 12:
                                if (pos + 1 > s_data_bytes.length) {
                                    System.out.println("数据不足以读取类型 1/12");
                                    break;
                                }
                                int val1 = Byte.toUnsignedInt(s_data_bytes[pos]);
                                System.out.printf("%d\n", val1);
                                pos += 1;
                                break;
                            case 2:
                                if (pos + 2 > s_data_bytes.length) {
                                    System.out.println("数据不足以读取类型 2");
                                    break;
                                }
                                int val2 = Short.toUnsignedInt(ByteBuffer.wrap(s_data_bytes, pos, 2).getShort());
                                System.out.printf("%d\n", val2);
                                pos += 2;
                                break;
                            case 3:
                            case 8:
                                if (pos + 4 > s_data_bytes.length) {
                                    System.out.println("数据不足以读取类型 3/8");
                                    break;
                                }
                                long val3 = Integer.toUnsignedLong(ByteBuffer.wrap(s_data_bytes, pos, 4).getInt());
                                System.out.printf("%d\n", val3);
                                pos += 4;
                                break;
                            case 4:
                            ...
                            case 6:
                            case 9:
                                if (pos + 8 > s_data_bytes.length) {
                                    System.out.println("数据不足以读取类型 4/5/6/9");
                                    break;
                                }
                                long val4 = ByteBuffer.wrap(s_data_bytes, pos, 8).getLong();
                                System.out.printf("%d\n", val4);
                                pos += 8;
                                break;
                            case 7:
                                if (pos + 4 > s_data_bytes.length) {
                                    System.out.println("数据不足以读取类型 7 的长度");
                                    break;
                                }
                                long len7 = Integer.toUnsignedLong(ByteBuffer.wrap(s_data_bytes, pos, 4).getInt());
                                System.out.printf("%d\n", len7);
                                pos += 4;
                                if (pos + len7 > s_data_bytes.length) {
                                    System.out.println("数据不足以读取类型 7 的数据");
                                    pos += len7; // 跳过
                                    break;
                                }
                                pos += len7;
                            ...
                            case 10:
                                if (pos + 1 > s_data_bytes.length) {
                                    System.out.println("数据不足以读取类型 10 的版本");
                                    break;
                                }
                                int ver = Byte.toUnsignedInt(s_data_bytes[pos]);
                                pos += 1;
                                if (ver == 4) {
                                    if (pos + 4 > s_data_bytes.length) {
                                        System.out.println("数据不足以读取类型 10 的 IPv4 地址");
                                        pos += 4;
                                        break;
                                    }
                                    pos += 4;
                                } else if (ver == 6) {
                                    if (pos + 16 > s_data_bytes.length) {
                                        System.out.println("数据不足以读取类型 10 的 IPv6 地址");
                                        pos += 16;
                                        break;
                                    }
                                    pos += 16;
                            ...
                            case 11:
                                if (pos + 4 > s_data_bytes.length) {
                                    System.out.println("数据不足以读取类型 11 的长度");
                                    break;
                                }
                                int len11 = (int) Integer.toUnsignedLong(ByteBuffer.wrap(s_data_bytes, pos, 4).getInt());
                                pos += 4;
                                if (pos + len11 > s_data_bytes.length) {
                                    System.out.println("数据不足以读取类型 11 的字符串");
                                    break;
                                }
                                String str = new String(s_data_bytes, pos, len11, StandardCharsets.UTF_8);
                                System.out.printf("%d %d\n%s\n", len11, s_data_bytes[pos + 4], str);
                                pos += len11;
                                break;
                            default:
                                System.out.printf("未知的字段类型: %d\n", f.type);
                                // 根据需要处理未知类型
                                break;
                        }
                    }
